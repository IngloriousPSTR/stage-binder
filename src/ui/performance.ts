...
export class PerformanceMode {
	...
	async open(
		...
		const overlay = document.body.createDiv({ cls: "sb-performance" });
		this.overlay = overlay;
		overlay.toggleClass("sb-perf-light", this.plugin.settings.perfTheme === "light");
		this.contentEl = overlay.createEl("main", { cls: "sb-perf-content sb-stage-main" });
		...
		overlay.toggleClass("sb-perf-noheader", !this.plugin.settings.perfShowHeader);
		const peek = overlay.createEl("button", { cls: "sb-perf-btn sb-perf-peek" });
		setIcon(peek, "more-horizontal");
		peek.setAttribute("aria-label", "Show controls");
		peek.addEventListener("click", () => {
			overlay.toggleClass("sb-perf-noheader", !overlay.hasClass("sb-perf-noheader"));
		});

		// Up next sits immediately above the performance dock.
		this.upNextEl = overlay.createDiv({ cls: "sb-perf-upnext" });
		this.buildDock(overlay);

		// Edge tap zones: generous touch targets that never cover the text's
		// center reading column.
		const tapPrev = overlay.createDiv({ cls: "sb-perf-tap sb-perf-tap-left" });
		tapPrev.addEventListener("click", () => void this.prev());
		const tapNext = overlay.createDiv({ cls: "sb-perf-tap sb-perf-tap-right" });
	...
	private buildDock(overlay: HTMLElement): void {
		const dock = overlay.createEl("nav", { cls: "sb-perf-dock" });
		...
		const navButton = (action: string, icon: string, text: string, label: string, handler: () => void) => {
			const button = dock.createEl("button", { cls: "sb-perf-dock-btn" });
			button.dataset.action = action;
			const iconEl = button.createSpan({ cls: "sb-perf-dock-icon" });
			setIcon(iconEl, icon);
		...
		this.scrollBtnEl.addClass("sb-scroll-btn");
		this.scrollIconEl = this.scrollBtnEl.querySelector<HTMLElement>(".sb-perf-dock-icon");
	...
	private async selectEntry(index: number): Promise<void> {
		...
		if (this.continuous) {
			const block = this.contentEl?.querySelector(`[data-sb-entry="${index}"]`);
	...
	private buildTools(): void {
		...
		chrome.setTools((panel) => {
			...
			if (entry?.fileKind === "song") {
				...
				chart.createSpan({ cls: "sb-stage-key", text: `Key: ${rendered?.chart.meta.key ?? entry.label ?? "—"}` });
			...
			text.createSpan({ cls: "sb-stage-value", text: `${this.plugin.settings.performanceZoom}%` });
			...
			this.speedEl = auto.createSpan({ cls: "sb-stage-value", text: String(this.effectiveSpeed()) });
	...
	private setTheme(value: "light" | "dark"): void {
		...
		this.overlay?.toggleClass("sb-perf-light", value === "light");
		void this.plugin.saveSettings();
		this.buildTools();
	}
	...
	private stepPdf(delta: -1 | 1): boolean | null {
		...
		const reader = this.contentEl?.querySelector<HTMLElement>(".sb-pdf-reader");
		if (!reader) return null;
		const detail: PdfPageStepDetail = { delta, found: false, moved: false };
		reader.dispatchEvent(new CustomEvent<PdfPageStepDetail>("sb-pdf-page", { detail }));
	...
	private async goSong(delta: number): Promise<void> {
		if (this.continuous) {
			...
			const block = el.querySelector(`[data-sb-entry="${target}"]`);
	...
	private jumpSection(dir: 1 | -1): boolean {
		...
		const anchors = Array.from(
			el.querySelectorAll<HTMLElement>(".label, .comment:not(.sb-stage-note), .sb-chart h1.title, .sb-service-card-title")
		);
	...
	private async render(): Promise<void> {
		...
		const applyCols = (el: HTMLElement) => {
			el.toggleClass("sb-cols-2", this.cols === 2);
			el.toggleClass("sb-cols-3", this.cols === 3);
		};

		if (this.continuous) {
			for (let i = 0; i < this.entries.length; i++) {
				const block = content.createDiv({ cls: "sb-perf-song" });
				block.setAttribute("data-sb-entry", String(i));
			...
			const target = content.querySelector(`[data-sb-entry="${this.index}"]`);
	...
	private async renderEntry(
		...
		if (entry.kind === "card" || !entry.file) {
			const card = container.createEl("article", { cls: "sb-service-card" });
			card.createDiv({ cls: "sb-service-card-time", text: this.context ? serviceEntryTime(this.context, entry) : "" });
			card.createEl("h1", { cls: "sb-service-card-title", text: entry.title });
			card.createDiv({ cls: "sb-service-card-meta", text: [entry.item.people.join(", "), serviceEntryDuration(entry)].filter(Boolean).join(" · ") });
			return null;
		}
	...
	private trackVisibleSong(): void {
		...
		const current = blockIndexAtTop(this.contentEl, "[data-sb-entry]");
	...
	private syncUpNext(): void {
		...
		el.toggleClass("sb-hidden", !show);
		if (!show) return;
		el.empty();

		const next = this.entries[this.index + 1];
		if (next) {
			if (this.context) el.createSpan({ cls: "sb-perf-upnext-clock", text: serviceEntryTime(this.context, next) });
			const length = serviceEntryDuration(next);
			el.createSpan({ cls: "sb-perf-upnext-text", text: `Next: ${next.title}${length ? ` (${length})` : ""}` });
		} else {
			el.createSpan({ cls: "sb-perf-upnext-text", text: "Last item" });
		}
	}
